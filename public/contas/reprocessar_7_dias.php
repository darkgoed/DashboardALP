...
if ($contaId <= 0) {
    Flash::error('Conta inválida para reprocessamento.');
    header('Location: ' . routeUrl('contas'));
    exit;
}
...
try {
    ...
    Flash::success($service->enqueueReprocessar($empresaId, $contaId, 7));
} catch (Throwable $e) {
    Flash::error('Erro ao enfileirar reprocessamento: ' . $e->getMessage());
}
