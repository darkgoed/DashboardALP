...
class MetaSyncService
{
    ...
    public function syncInsights(
        ...
        [$dataInicio, $dataFim] = $this->resolveMetaInsightsWindow(
    ...
    private function resolveMetaInsightsWindow(string $dataInicio, string $dataFim): array
    {
        ...
        $inicioLimite = $fim->modify('-37 months +1 day');
        $inicio = new DateTimeImmutable($dataInicio);

        if ($inicio < $inicioLimite) {
            $inicio = $inicioLimite;
        }
    ...
    public function syncManutencao(array $options = []): array
    {
        ...
        $reencaminhados = $syncJob->requeueStuckJobs(30);

        $this->log(
            (int) ($options['empresa_id'] ?? 0),
            null,
            null,
            'sync_manutencao',
            'success',
            'Rotina de manutenção executada.',
            json_encode([
                'jobs_reencaminhados' => $reencaminhados,
                'executado_em' => date('Y-m-d H:i:s'),
            ], JSON_UNESCAPED_UNICODE),
            $options['sync_job_id'] ?? null
        );
    ...
    private function syncCampaigns(int $empresaId, int $contaId, string $metaAccountId, string $token): array
    {
        ...
        $map = [];
        $totalRecebido = count($response['data'] ?? []);
        ...
        $this->log(
            ...
            null,
            $contaId,
            'sync_estrutura_campaigns',
            'success',
            'Campanhas processadas',
            json_encode([
                'meta_account_id' => $metaAccountId,
                'recebidas' => $totalRecebido,
                'gravadas' => count($map),
            ], JSON_UNESCAPED_UNICODE)
        );
    ...
    {
        // MySQL UNIQUE indexes with NULL columns do not reliably collide for
        // campaign/adset rows, so repeated sync jobs can accumulate duplicates.
        // Remove any existing rows for the same natural key before inserting.
        $deleteSql = "
            DELETE FROM insights_diarios
            WHERE empresa_id = :empresa_id
              AND cliente_id = :cliente_id
              AND conta_id = :conta_id
              AND campanha_id <=> :campanha_id
              AND conjunto_id <=> :conjunto_id
